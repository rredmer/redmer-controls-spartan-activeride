...
Begin VB.Form ErrorForm 
   ...
   Begin VB.Label TitleLabel 
      BackStyle       =   0  'Transparent
      Caption         =   "Error has occurred in the program.  The details of this"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
...
Private Sub Form_Load()
    ...
    ViewLog.Log DebugMsg, "Loading Error Handling Form."
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    ViewLog.Log DebugMsg, "Unloading Error Handling Form."
...
Public Sub ReportError(ErrProcName As String, ErrNum As Integer, ErrDllNum As Integer, ErrSource As String, ErrDescript As String, ErrShowWin As Boolean)
    ...
    ErrMsg = ""
    ErrMsg = ErrMsg & "Report time.: " & Now & vbCrLf                       'The time the error occurred
    ErrMsg = ErrMsg & "Error number: " & Trim$(Str$(ErrNum)) & vbCrLf         'The VB Error Number
    ErrMsg = ErrMsg & "DLL Error...: " & Trim$(Str$(ErrDllNum)) & vbCrLf      'The last DLL Error that occurred
    ErrMsg = ErrMsg & "In procedure: " & ErrProcName & vbCrLf               'The Procure in which the error occurred
    ...
    ViewLog.Log ErrorMsg, "Error (" & Trim$(Str$(ErrNum)) & ")(" & Trim$(Str$(ErrDllNum)) & ") In Procedure (" & ErrProcName & ") source (" & ErrSource & ") description (" & ErrDescript & ")."
    ErrMsg = ""
    Exit Sub

ErrorHandler:
    MsgBox "An error occurred formatting the error message.  Please contact technical support.", vbApplicationModal + vbOKOnly + vbInformation, "Error"
End Sub
...
Private Sub ToolBar_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Exit Sub
ErrorHandler:
    MsgBox "An error occurred in the Error Toolbar.  Please contact technical support.", vbApplicationModal + vbOKOnly + vbInformation, "Error"
End Sub
...
Private Sub SaveErrorTextToFile()
    ...
    Exit Sub
ErrorHandler:
    MsgBox "An error occurred writing to text file.  Please contact technical support.", vbApplicationModal + vbOKOnly + vbInformation, "Error"
End Sub
